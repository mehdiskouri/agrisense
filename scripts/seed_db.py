    ...
    payloads = [
        {
            ...
            "growth_stages": {
                "seedling": {
                    "duration_days": 14,
                    "water_demand_mm_day": 2.5,
                    "npk_demand": {"n": 35, "p": 18, "k": 28},
                },
                "vegetative": {
                    "duration_days": 30,
                    "water_demand_mm_day": 4.5,
                    "npk_demand": {"n": 55, "p": 25, "k": 45},
                },
                "fruiting": {
                    "duration_days": 35,
                    "water_demand_mm_day": 5.8,
                    "npk_demand": {"n": 45, "p": 30, "k": 60},
                },
            },
        },
        {
            ...
            "growth_stages": {
                "seedling": {
                    "duration_days": 10,
                    "water_demand_mm_day": 2.0,
                    "npk_demand": {"n": 25, "p": 16, "k": 20},
                },
                "leafing": {
                    "duration_days": 25,
                    "water_demand_mm_day": 3.2,
                    "npk_demand": {"n": 35, "p": 20, "k": 28},
                },
            },
        },
        {
            ...
            "growth_stages": {
                "seedling": {
                    "duration_days": 14,
                    "water_demand_mm_day": 2.3,
                    "npk_demand": {"n": 30, "p": 16, "k": 24},
                },
                "flowering": {
                    "duration_days": 30,
                    "water_demand_mm_day": 4.2,
                    "npk_demand": {"n": 40, "p": 24, "k": 45},
                },
                "fruiting": {
                    "duration_days": 36,
                    "water_demand_mm_day": 5.2,
                    "npk_demand": {"n": 38, "p": 28, "k": 55},
                },
            },
        },
    ]
...
def _build_topology(synthetic: dict[str, Any]) -> SeedArtifacts:
    ...
    for zone in zone_defs:
        ...
        zone_type = (
            ...
            if zone["zone_type"] == "greenhouse"
            else ZoneTypeEnum.open_field
        )
        zone_types[zone_token] = zone_type
        zone_area_m2[zone_token] = (
            greenhouse_area if zone_type == ZoneTypeEnum.greenhouse else open_field_area
        )
    ...
    weather_station_ids = [
        _stable_uuid(f"vertex:weather_station:{token}") for token in station_names
    ]
    ...
    fixture_zones = (
        [greenhouse_zones[index % len(greenhouse_zones)] for index in range(n_fixtures)]
        if greenhouse_zones
        else []
    )
...
async def _seed_topology(
    session: Any, synthetic: dict[str, Any], artifacts: SeedArtifacts
) -> dict[str, int]:
    ...
    for zone_token in zone_order:
        session.add(
            Zone(
                ...
                soil_type="loam"
                if artifacts.zone_types[zone_token] == ZoneTypeEnum.greenhouse
                else "sandy_loam",
    ...
    for sensor_id, zone_token in zip(
        artifacts.soil_sensor_ids, artifacts.soil_sensor_zones, strict=True
    ):
    ...
    for zone_token in zone_order:
        ...
        zone_fixtures = [
            fixture
            for fixture, fixture_zone in zip(
                artifacts.fixture_ids, artifacts.fixture_zones, strict=True
            )
            if fixture_zone == zone_token
        ]
        zone_beds = [
            bed
            for bed, bed_zone in zip(artifacts.bed_ids, artifacts.bed_zones, strict=True)
            if bed_zone == zone_token
        ]
        ...
        edges: list[tuple[HyperEdgeLayerEnum, list[uuid.UUID], dict[str, Any]]] = [
            ...
            (
                HyperEdgeLayerEnum.weather,
                [*artifacts.weather_station_ids, *zone_soil],
                {"zone_id": str(zone_id)},
            ),
        ...
        if zone_beds and zone_cameras:
            edges.append(
                (HyperEdgeLayerEnum.vision, [*zone_beds, *zone_cameras], {"zone_id": str(zone_id)})
            )
...
def _soil_rows(
    synthetic: dict[str, Any], artifacts: SeedArtifacts, timestamps: list[datetime]
    ...
    for step, timestamp in enumerate(timestamps):
        for sensor_index, sensor_id in enumerate(artifacts.soil_sensor_ids):
            ...
            yield {
                ...
                "conductivity": None
                if _is_missing(conductivity_value)
                else float(conductivity_value),
...
def _weather_rows(
    ...
    for step, timestamp in enumerate(timestamps):
        for station_index, station_id in enumerate(artifacts.weather_station_ids):
            ...
            if (
                _is_missing(temperature_value)
                or _is_missing(humidity_value)
                or _is_missing(precipitation_value)
            ):
                continue
            ...
            yield {
                ...
                "wind_direction": None
                if _is_missing(wind_direction_value)
                else float(wind_direction_value),
...
def _npk_rows(
    synthetic: dict[str, Any], artifacts: SeedArtifacts, days: int
) -> Iterable[dict[str, Any]]:
...
async def seed_database() -> dict[str, Any]:
    ...
    async with async_session_factory() as session:
        ...
        counts["soil_readings"] = await _bulk_insert(
            session, SoilReading, _soil_rows(synthetic, artifacts, timestamps)
        )
        timings["soil_ms"] = round((perf_counter() - soil_start) * 1000.0, 2)

        weather_start = perf_counter()
        counts["weather_readings"] = await _bulk_insert(
            session, WeatherReading, _weather_rows(synthetic, artifacts, timestamps)
        )
        timings["weather_ms"] = round((perf_counter() - weather_start) * 1000.0, 2)

        irrigation_start = perf_counter()
        counts["irrigation_events"] = await _bulk_insert(
            session,
            IrrigationEvent,
            _irrigation_rows(synthetic, artifacts, timestamps, zone_tokens),
        )
        timings["irrigation_ms"] = round((perf_counter() - irrigation_start) * 1000.0, 2)

        npk_start = perf_counter()
        counts["npk_samples"] = await _bulk_insert(
            session, NpkSample, _npk_rows(synthetic, artifacts, days=days)
        )
        timings["npk_ms"] = round((perf_counter() - npk_start) * 1000.0, 2)

        lighting_start = perf_counter()
        counts["lighting_readings"] = await _bulk_insert(
            session, LightingReading, _lighting_rows(synthetic, artifacts, timestamps)
        )
        timings["lighting_ms"] = round((perf_counter() - lighting_start) * 1000.0, 2)

        vision_start = perf_counter()
        counts["vision_events"] = await _bulk_insert(
            session, VisionEvent, _vision_rows(synthetic, artifacts, timestamps)
        )
...
async def _async_main() -> None:
    try:
        ...
        sys.stderr.write("=== AgriSense Seed Report ===\n")
        sys.stderr.write("status: failed\n")
        sys.stderr.write(f"error: {exc}\n")
        sys.stderr.write(
            "hint: ensure PostgreSQL/Redis are running and DATABASE_URL/REDIS_URL are correct\n"
        )
        raise SystemExit(1) from exc
    finally:
        await engine.dispose()

    sys.stdout.write("=== AgriSense Seed Report ===\n")
    sys.stdout.write(f"status: {report['status']}\n")
    sys.stdout.write(f"farm_id: {report['farm_id']}\n")
    sys.stdout.write(
        f"duration_s: {report['duration_s']} (target<30s: {report['target_under_30s']})\n"
    )
    sys.stdout.write("counts:\n")
    for key, value in sorted(report["counts"].items()):
        sys.stdout.write(f"  - {key}: {value}\n")
    sys.stdout.write("timings_ms:\n")
