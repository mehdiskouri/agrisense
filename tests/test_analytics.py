...
@pytest.mark.asyncio
async def test_analytics_status_endpoint_success(
    ...
    async def fake_status(self: AnalyticsService, _farm_id: object) -> FarmStatusResponse:
        return FarmStatusResponse(
            ...
            zones=[
                ZoneStatus(
                    zone_id=zone_id, query_vertex_id=vertex_id, status={"soil": {"ok": True}}
                )
            ],
        )
...
@pytest.mark.asyncio
async def test_analytics_zone_detail_endpoint_supports_vertex(
    client: AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    farm_id = uuid4()
    zone_id = uuid4()
    ...
    async def fake_detail(
        self: AnalyticsService, _farm_id: object, _query: ZoneDetailQuery
    ) -> ZoneDetailResponse:
    ...
    response = await client.get(
        f"/api/v1/analytics/{farm_id}/zones/{zone_id}?vertex_id={vertex_id}"
    )
    assert response.status_code == 200
    body = response.json()
...
@pytest.mark.asyncio
async def test_analytics_vertex_detail_endpoint(
    client: AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    farm_id = uuid4()
    vertex_id = uuid4()
    ...
    async def fake_detail(
        self: AnalyticsService, _farm_id: object, query: ZoneDetailQuery
    ) -> ZoneDetailResponse:
...
@pytest.mark.asyncio
async def test_alerts_endpoint_and_openapi_contract(
    client: AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    farm_id = uuid4()
    zone_id = uuid4()
