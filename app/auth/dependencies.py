...
def _coerce_scopes(raw: Mapping[str, Any] | None) -> set[str]:
    if raw is None:
        return set()
    scopes: set[str] = set()
...
async def _resolve_user_from_token(
    ...
    try:
        ...
        raise _raise_auth(
            AuthError(code="token_invalid", detail="Token subject is invalid")
        ) from exc
...
def require_role(*allowed: UserRoleEnum) -> Callable[[User], Awaitable[User]]:
