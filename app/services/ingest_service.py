...
class IngestService:
    ...
    async def ingest_soil(
        self, farm_id: uuid.UUID, readings: Sequence[SoilReadingIn]
    ) -> IngestReceipt:
        ...
        for idx, item in enumerate(readings):
            ...
            if vertex.config and vertex.config.get("sensor_type") not in (None, "soil"):
                warnings.append(
                    IngestWarning(index=idx, message="sensor_type mismatch for soil endpoint")
                )
    ...
    async def ingest_weather(
        self, farm_id: uuid.UUID, readings: Sequence[WeatherReadingIn]
    ) -> IngestReceipt:
    ...
    async def ingest_irrigation(
        ...
        for idx, item in enumerate(events):
            ...
            flow_rate = (
                float(item.volume_liters or 0.0) / duration_minutes
                if duration_minutes > 0
                else float(item.volume_liters or 0.0)
            )
    ...
    async def ingest_npk(self, farm_id: uuid.UUID, samples: Sequence[NpkSampleIn]) -> IngestReceipt:
        ...
        for idx, item in enumerate(samples):
            ...
            if target_vertex is None:
                warnings.append(
                    IngestWarning(
                        index=idx, message="no sensor vertex found in zone for npk graph update"
                    )
                )
                event_details.append(
                    {
    ...
    async def ingest_vision(
        self, farm_id: uuid.UUID, events: Sequence[VisionEventIn]
    ) -> IngestReceipt:
            ...
            event_details.append(
                {
                    ...
                    if crop_bed_vertex.zone_id is not None
                    else None,
    ...
    async def ingest_lighting(
        ...
        for idx, item in enumerate(readings):
            ...
            if normalized != "lighting":
                warnings.append(
                    IngestWarning(
                        index=idx, message="lighting payload received unsupported layer alias"
                    )
                )

            vertex = await self._require_vertex(
                item.fixture_id, farm_id, {VertexTypeEnum.light_fixture}
            )
            row = LightingReading(
    ...
    async def _require_vertex(
        ...
        if vertex.vertex_type not in allowed_types:
            ...
            raise ValueError(
                f"vertex {vertex_id} has invalid type {vertex.vertex_type}; "
    ...
    async def _validate_active_layer(self, farm_id: uuid.UUID, layer: str) -> None:
        ...
        normalized_active = {
            self.normalize_layer(token) for token in active_layers if token in LAYER_ALIASES
        }
